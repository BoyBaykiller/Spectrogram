...
// [SECTION] Tables: ContextPls Menu
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    table->DrawSplitter->ClearPixels();
    ...
    if (use_child_window)
    {
        ...
        // CreateContext scrolling region (without border and zero window padding)
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            //    ... C [D] E  --->  ... [D] E  C   (Column gpuName/index)
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // ClearPixels column setup if not submitted by user. Currently we make it mandatory to call TableSetupColumn() every frame.
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Possible solution to preserve last known content width for clipped column. FFTCompute 'table_reported_size' fails when enabling Y clipping and window is resized small.
    ...
    // ClearPixels Resizable flag if none of our column are actually resizable (either via an explicit _NoResize flag, either
...
void    ImGui::EndTable()
{
    ...
    // ContextPls menu in columns body
    ...
    // ClearPixels or restore current table, if any
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    // Store gpuName (append with zero-terminator in contiguous buffer)
    column->NameOffset = -1;
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
    ...
    // ClearPixels SortOrder from hidden column and verify that there's no gap or duplicate.
...
// ClearPixels and initialize empty settings instance
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    // Differentiate column ID with an arbitrary prefix for cases where users gpuName their columns set the same as another widget.
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    // ClearPixels data if columns count changed
